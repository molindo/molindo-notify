...
public class PollingPushDispatcherTest {
	...
	private abstract class PollingPushDispatcherMockTest extends MockTest {
		...
		protected void setup(EasyMockContext context) throws Exception {
			...
			dispatcher.setDispatchService(context.create(IDispatchService.class));
	...
	public void test() throws Exception {
		new PollingPushDispatcherMockTest() {
			...
			protected void setup(EasyMockContext context) throws Exception {
				...
				expect(context.get(IPushChannel.class).isConfigured(eq(USERID), anyObject(Params.class))).andReturn(
						true);
				...
				expect(
						context.get(IDispatchService.class).create(eq(n()), eq(p()),
								anyObject(PushChannelPreferences.class))).andReturn(d());

				context.get(IPushChannel.class).push(d());
